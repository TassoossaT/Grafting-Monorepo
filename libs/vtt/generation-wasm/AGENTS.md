...
Consumed by `apps/architecture-studio`'s `/vtt-generation` route as a
normal `workspace:*` dependency on this crate's own co-located
`package.json` (`@grafting/vtt-generation-wasm`) -- not a separate
...
NOT be committed. Do not reintroduce a separate `packages/vtt-generation-wasm`
technical package.
