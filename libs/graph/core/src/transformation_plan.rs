...
impl<I: Ord> IdentityDelta<I> {
    ...
    pub fn new(
        ...
        Ok(Self { created, preserved, replaced, removed })
    }

    /// Identities newly introduced by the plan.
    pub fn created(&self) -> &BTreeSet<I> { &self.created }
    /// Identities retained because they still connect unaffected geometry.
    pub fn preserved(&self) -> &BTreeSet<I> { &self.preserved }
    /// Identities superseded inside the remodeled region.
    pub fn replaced(&self) -> &BTreeSet<I> { &self.replaced }
    /// Identities removed by the plan.
    pub fn removed(&self) -> &BTreeSet<I> { &self.removed }
    /// Whether this category has no lifecycle changes.
    pub fn is_empty(&self) -> bool { self.created.is_empty() && self.preserved.is_empty() && self.replaced.is_empty() && self.removed.is_empty() }
}
...
impl LocalInvalidationScope {
    ...
    pub fn new(changed_surfaces: BTreeSet<SurfaceKey>, topology_repair_neighbors: BTreeSet<SurfaceKey>, direct_dependencies: BTreeSet<SurfaceKey>) -> Self {
        Self { changed_surfaces, topology_repair_neighbors, direct_dependencies }
    }
    /// Surfaces directly changed by the plan.
    pub fn changed_surfaces(&self) -> &BTreeSet<SurfaceKey> { &self.changed_surfaces }
    /// Adjacent surfaces inspected for topology repair or fragment cleanup.
    pub fn topology_repair_neighbors(&self) -> &BTreeSet<SurfaceKey> { &self.topology_repair_neighbors }
    /// Direct transformer dependencies, excluding unrelated clouds.
    pub fn direct_dependencies(&self) -> &BTreeSet<SurfaceKey> { &self.direct_dependencies }
    /// Whether the plan identifies no local refresh work.
    pub fn is_empty(&self) -> bool { self.changed_surfaces.is_empty() && self.topology_repair_neighbors.is_empty() && self.direct_dependencies.is_empty() }
}
...
impl TransformationPlan {
    ...
    pub fn new(node_ids: IdentityDelta<NodeId>, edge_ids: IdentityDelta<EdgeId>, surface_ids: IdentityDelta<SurfaceKey>, invalidation: LocalInvalidationScope) -> Result<Self, TransformationPlanFailure> {
        if node_ids.is_empty() && edge_ids.is_empty() && surface_ids.is_empty() { return Err(TransformationPlanFailure::NoChanges); }
        if invalidation.is_empty() { return Err(TransformationPlanFailure::EmptyInvalidationScope); }
        Ok(Self { node_ids, edge_ids, surface_ids, invalidation })
    }
    /// Node lifecycle changes.
    pub fn node_ids(&self) -> &IdentityDelta<NodeId> { &self.node_ids }
    /// Edge lifecycle changes.
    pub fn edge_ids(&self) -> &IdentityDelta<EdgeId> { &self.edge_ids }
    /// Surface lifecycle changes.
    pub fn surface_ids(&self) -> &IdentityDelta<SurfaceKey> { &self.surface_ids }
    /// Local derived-state refresh scope.
    pub fn invalidation(&self) -> &LocalInvalidationScope { &self.invalidation }
}
