...
pub fn cut_region(
    ...
    if cut_path.is_empty() {
        return Err(ContourError::EmptyLoop.into());
    }

    let boundary = &source.outer_loops()[0];
    let cut_start = use_start(topology, &cut_path[0])?;
    let cut_end = use_end(topology, &cut_path[cut_path.len() - 1])?;

    let starts: Vec<NodeId> = boundary
        .iter()
        .map(|use_| use_start(topology, use_))
        .collect::<Result<_, _>>()?;
    let (Some(at_end), Some(at_start)) = (
        starts.iter().position(|node| *node == cut_end),
        starts.iter().position(|node| *node == cut_start),
    ) else {
        return Err(RegionEditError::CutEndpointsNotOnBoundary {
            region: region.clone(),
        });
    };

    // Walking the boundary from the cut's end back around to its start is
    // one half; the complementary walk is the other. Closing each with the
    // cut itself (forward on one side, reversed on the other) yields two
    // closed loops sharing that one boundary.
    let first_half = rotate_slice(boundary, at_end, at_start);
    let second_half = rotate_slice(boundary, at_start, at_end);

    let mut first_loop = first_half;
    first_loop.extend(cut_path.iter().cloned());
    let mut second_loop = second_half;
    second_loop.extend(cut_path.iter().rev().map(flip));

    let surface = surfaces
        .region_surface(region)
        .map(|surface| (surface.surface_type().clone(), surface.physical()));

    topology.remove_region(region)?;
    if surface.is_some() {
        surfaces.remove_region_surface(region)?;
    }
    topology.add_region(first.clone(), vec![first_loop], Vec::new())?;
    topology.add_region(second.clone(), vec![second_loop], Vec::new())?;
    if let Some((surface_type, physical)) = surface {
        surfaces.add_region_surface(topology, first.clone(), surface_type.clone(), physical)?;
        surfaces.add_region_surface(topology, second.clone(), surface_type, physical)?;
    }
    Ok(RegionEditOutcome {
        created_regions: vec![first, second],
        removed_regions: vec![region.clone()],
        ..RegionEditOutcome::default()
    })
}

/// The uses of `loop_` starting at index `from` and running forward (wrapping
/// once) up to, but not including, index `to`.
fn rotate_slice(loop_: &ContourLoop, from: usize, to: usize) -> ContourLoop {
    let length = loop_.len();
    let mut walked = Vec::new();
    let mut index = from;
    while index != to {
        walked.push(loop_[index].clone());
        index = (index + 1) % length;
    }
    walked
}

fn flip(use_: &OrientedEdgeUse) -> OrientedEdgeUse {
    if use_.is_reversed() {
        OrientedEdgeUse::forward(use_.edge().clone())
    } else {
...
fn use_start(
    topology: &ContourTopology,
    use_: &OrientedEdgeUse,
) -> Result<NodeId, RegionEditError> {
    let edge = topology
        .edge(use_.edge())
        .ok_or_else(|| ContourError::UnknownEdge {
            id: use_.edge().clone(),
        })?;
    Ok(if use_.is_reversed() {
        edge.end_node().clone()
    } else {
...
fn use_end(topology: &ContourTopology, use_: &OrientedEdgeUse) -> Result<NodeId, RegionEditError> {
    let edge = topology
        .edge(use_.edge())
        .ok_or_else(|| ContourError::UnknownEdge {
            id: use_.edge().clone(),
...
mod tests {
    ...
    fn add_and_remove_hole_run_through_the_same_validation_as_an_outer_loop() {
        ...
        for (id, position) in [
            ...
            graph.add_node(Node::new(nid(id), position)).unwrap();
        }
        let hole: ContourLoop = names
            .iter()
            .enumerate()
            .map(|(index, start)| {
                let end = names[(index + 1) % names.len()];
                let edge_id = eid(&format!("hole-{index}"));
                topology
                    .add_edge(
                        &graph,
                        ContourEdge::new(
        ...
        assert_eq!(
            ...
            vec![nid("h0"), nid("h1"), nid("h2"), nid("h3")],
            "a removed hole's own nodes are orphans and must not survive"
        );
    }
    ...
    #[test]
    fn cut_region_splits_one_region_into_two_sharing_the_cut_as_a_manifold_boundary() {
        let (mut graph, mut topology, mut surfaces, region) = quad();
        // The two crossing points the cut runs between, inserted exactly as
        // a T-junction weld would insert them.
        insert_vertex(
            &mut graph,
            &mut topology,
            &eid("quad-0"),
            Node::new(nid("bottom"), [0.5, 0.0, 0.0]),
            eid("a-bottom"),
            eid("bottom-b"),
        )
        .unwrap();
        ...
        topology
            .add_edge(
                &graph,
                ContourEdge::new(eid("cut"), nid("bottom"), nid("top"), ContourGeometry::Line),
            )
            .unwrap();
        ...
        let outcome = cut_region(
            ...
            &mut surfaces,
            &region,
            &[OrientedEdgeUse::forward(eid("cut"))],
            rid("left"),
            rid("right"),
        )
        .unwrap();
        ...
        assert_eq!(outcome.removed_regions, vec![region.clone()]);
        assert_eq!(outcome.created_regions, vec![rid("left"), rid("right")]);
        assert!(topology.region(&region).is_none());
        assert_eq!(
            topology.regions_using_edge(&eid("cut")),
            vec![rid("left"), rid("right")],
            "the cut is one shared edge walked in opposite directions, not two coincident edges"
        );
    ...
    #[test]
    fn cut_region_rejects_a_path_that_does_not_land_on_the_boundary() {
        let (mut graph, mut topology, mut surfaces, region) = quad();
        graph
            .add_node(Node::new(nid("floating"), [0.5, 0.0, 0.5]))
            .unwrap();
        topology
            .add_edge(
                &graph,
                ContourEdge::new(eid("cut"), nid("a"), nid("floating"), ContourGeometry::Line),
            )
            .unwrap();
        let error = cut_region(
            &mut topology,
            &mut surfaces,
            &region,
            &[OrientedEdgeUse::forward(eid("cut"))],
        ...
        assert_eq!(
            error,
            RegionEditError::CutEndpointsNotOnBoundary {
                region: region.clone()
            }
        );
        assert!(
            topology.region(&region).is_some(),
            "a rejected cut leaves the original untouched"
        );
    }
