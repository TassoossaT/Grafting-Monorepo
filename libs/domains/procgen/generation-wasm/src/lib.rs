...
/// `origin_x` / `origin_y` place the grid's first cell in the noise's own
/// coordinates, so a caller can ask for the window it needs instead of always
/// the one starting at the origin.
///
/// Without it a caller has only one patch of noise and has to stretch it over
/// whatever extent it happens to be filling -- which means the same point in
/// the world takes a different height every time a differently sized request
/// covers it. Two patches of ground generated by two such requests meet along
/// a crease, and no amount of agreeing about *cells* fixes it, because the
/// disagreement is about *height*. Anchoring the window is what lets ground
/// generated at different moments be the same surface.
#[wasm_bindgen]
...
mod tests {
    ...
    fn produces_one_value_per_cell_in_range() {
        let values = generate_heightmap(8, 6, 42, 0.15, 0, 0).expect("0.15 is a valid scale");
    ...
    fn is_deterministic_for_a_given_seed() {
        let a = generate_heightmap(4, 4, 7, 0.2, 0, 0).expect("0.2 is a valid scale");
        let b = generate_heightmap(4, 4, 7, 0.2, 0, 0).expect("0.2 is a valid scale");
        assert_eq!(a, b);
    }

    #[test]
    fn differs_across_seeds() {
        let a = generate_heightmap(4, 4, 1, 0.2, 0, 0).expect("0.2 is a valid scale");
        let b = generate_heightmap(4, 4, 2, 0.2, 0, 0).expect("0.2 is a valid scale");
        assert_ne!(a, b);
    }
    ...
    fn a_permitted_scale_still_varies_across_the_grid() {
        ...
        let values = generate_heightmap(16, 16, 1, 0.15, 0, 0).expect("0.15 is a valid scale");
        let first = values[0];
        assert!(
