...
`duplicate_surface`) and `grafting-procgen-terrain-generation`'s pure
generators, wired into one
...
`terrain-generation`'s own "never mutates a graph"
...
construction ops, terrain-generation) are VTT- or
