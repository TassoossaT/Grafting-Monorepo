...
/// JSON boundary for a directed planar curve; all geometry stays in graph-core.
#[derive(Debug, Clone, Deserialize, Serialize)]
pub struct PlanarCurveDto {
    pub start: [f32; 2],
    pub end: [f32; 2],
    pub geometry: ContourGeometryDto,
}
/// Runs an analytic boolean while keeping vendor/core types behind the wire DTO.
pub fn curved_planar_boolean(
    subject: Vec<Vec<Vec<PlanarCurveDto>>>,
    clip: Vec<Vec<Vec<PlanarCurveDto>>>,
    operation: grafting_graph_core::PlanarBoolean,
) -> Result<Vec<Vec<Vec<PlanarCurveDto>>>, String> {
    let convert = |shapes: Vec<Vec<Vec<PlanarCurveDto>>>| {
        shapes
            .into_iter()
            .map(|s| {
                s.into_iter()
                    .map(|r| {
                        r.into_iter()
                            .map(|c| grafting_graph_core::PlanarCurve {
                                start: c.start,
                                end: c.end,
                                geometry: c.geometry.into_geometry(),
                            })
                            .collect()
                    })
                    .collect()
    ...
    let result =
        grafting_graph_core::curved_planar_boolean(&convert(subject), &convert(clip), operation)?;
    Ok(result
        .into_iter()
        .map(|s| {
            s.into_iter()
                .map(|r| {
                    r.into_iter()
                        .map(|c| PlanarCurveDto {
                            start: c.start,
                            end: c.end,
                            geometry: ContourGeometryDto::from_geometry(&c.geometry),
                        })
                        .collect()
                })
                .collect()
