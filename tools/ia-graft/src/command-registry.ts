...
export const COMMAND_REGISTRY: AnyCommand[] = [
  ...
  defineCommand<TaskDiffInput>({
    name: "graft_task_diff",
    group: "task",
    subcommand: "diff",
    description: "Inspects uncommitted or branch changes inside the task worktree with token-capped output.",
    parameters: {
      taskId: { type: "string", description: "Target task ID", required: true, flag: "--id" },
      stat: { type: "boolean", description: "Return diffstat summary instead of full diff" },
      staged: { type: "boolean", description: "Inspect staged changes instead of unstaged changes" },
      base: { type: "boolean", description: "Diff all task branch commits against base branch" },
      files: { type: "array", description: "Limit diff to specific file paths", flag: "--file" },
    },
    handler: (root, input) => taskDiff(root, input),
  }),
