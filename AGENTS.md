...
- **Any task**: run `ia-graft task new --id <TASK-ID>`. This creates an
  isolated Git worktree and branch — work only inside that worktree.
...
- When the task is complete, run `ia-graft task done --id <TASK-ID> --title
  ...
  `gh`. If `gh` can't open one (missing/unauthenticated), it still pushes and
  returns a manual compare URL instead of failing — open that URL yourself.
  After the PR merges, run `ia-graft task cleanup --id <TASK-ID>` to remove
  the worktree.
