...
- **Code Autonomy & Boundaries:**
  ...
  - MUST NOT couple generic code to one consumer (owner rule, 2026-09-25): every function, method and behavior is written so a future type, tool or app can reuse it unchanged.
    - Put logic at the most generic layer that can own it. Shared math and geometry go in Rust/`grafting-graph-core` or a shared module; per-type differences are capabilities a type **declares** and **one** generic dispatcher reads. Generic code MUST NOT import a specific type's module or name.
    - Search before writing. Extend the existing function instead of adding a sibling. The same walk, algorithm, commit sequence or id/prefix check MUST exist in exactly one place.
    - Name modules and functions for the capability, not their first consumer. No `road*` or `slope*` names on code any spine, surface or tool could use.
