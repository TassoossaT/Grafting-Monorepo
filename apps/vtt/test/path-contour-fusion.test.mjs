...
function mouthInto(run, stationZ = [-6, -3, 0], pivot = JUNCTION.nodeId) {
  // The weld the spine join left behind: this run's last station is the
  // junction node on the standing spine.
  const welds = new Map([[`${stationZ.length - 1}:0`, pivot]]);
  return pathMouthsInto(arrivingPlan(stationZ), 3, 1, [run], welds);
}
...
test("closing the mouth takes out the flank it opens into", () => {
  ...
  const wedges = junctionWedges(TABLE, "op-2", ARRIVING, [mouth]);
...
test("a junction node the run does not carry rebuilds nothing, rather than half", () => {
  const run = standingRun();
  const [mouth] = mouthInto(run).mouths;
  const stranger = stationNodeId(STANDING, 9.5, 0);
  const [strayed] = mouthInto(run, [-6, -3, 0], stranger).mouths;

  // One wedge alone leaves the flank it replaced half open, which is worse
  // than the kerb it was meant to remove.
  assert.ok(mouth !== undefined);
  assert.equal(junctionWedges(TABLE, "op-2", ARRIVING, [strayed]), undefined);
});
