...
/**
 * A road authored click by click, committed one span per click exactly as
 * `path-line-tool.ts` does -- the anchor for the next span is the point the
 * last click landed on.
 */
function click(f, points, prefix) {
  for (let i = 0; i + 1 < points.length; i += 1) {
    const effect = createPathBrushEffect({
      brushShape: { kind: "circle", radius: 0.5 },
      brushRegion: { samples: [points[i], points[i + 1]] },
      referenceLine: "authored",
      parameters: pathFormationFor(road),
    }, { operationId: `${prefix}:${i}`, tableId: "bezier-test", initiatedBy: "path-line" });
    const plan = planPathCloudMutation({ bezier: f.runtime, tableId: "bezier-test", snapToGrid: false,
      graphSnapshot: f.runtime.getGraphSnapshot(), regionTopologies: f.runtime.getAllRegionTopologies(),
...
test("real WASM: a clicked road puts an anchor exactly where every click landed", () => {
  const f = sessionFixture();
  try {
    // Three clicks, so the middle one is a turn the fit would have been free
    // to move, round off, or drop entirely.
    click(f, [point(-10, 0), point(0, 0), point(0, 10)], "clicked");
    const spine = f.runtime.getGraphSnapshot().nodes.filter((n) => n.id.startsWith("spine:"));
    for (const wanted of [point(-10, 0), point(0, 0), point(0, 10)]) {
      assert.ok(
        spine.some((n) => n.position.x === wanted.x && n.position.z === wanted.z),
        `no anchor at ${wanted.x},${wanted.z} -- authored points must survive verbatim`,
      );
    }
    assert.equal(curves(f).length, 2);
  } finally { f.session.free(); }
...
test("real WASM: a clicked turn stays exactly as clicked, on both spans", () => {
  const f = sessionFixture();
  try {
    click(f, [point(-10, 0), point(0, 0), point(0, 10)], "corner");
    const edges = curves(f);
    assert.equal(edges.length, 2);
    // Straight is a cubic whose controls lie a third along its own chord.
    // Neither span may drift from that: not the new one, which would mean
    // the weld bent what was just authored, and not the standing one, which
    // would mean a later click reshaped a road already committed.
    const nodes = new Map(f.runtime.getGraphSnapshot().nodes.map((n) => [n.id, n.position]));
    for (const e of edges) {
      const a = nodes.get(e.startNodeId);
      const b = nodes.get(e.endNodeId);
      const third = [(b.x - a.x) / 3, (b.y - a.y) / 3, (b.z - a.z) / 3];
      for (const [actual, wanted] of [[e.curve.start, third], [e.curve.end, third.map((v) => -v)]]) {
        assert.ok(
          Math.hypot(actual[0] - wanted[0], actual[1] - wanted[1], actual[2] - wanted[2]) < 1e-6,
          `${e.edgeId} was bent: ${JSON.stringify(actual)} vs ${JSON.stringify(wanted)}`,
        );
      }
    }
  } finally { f.session.free(); }
