 ...
 *
 * Algorithm: every wall-brush wall is an edge between its two bottom corner
 ...
 * edge, keeps whichever closed loops actually contain the click point
 * (point-in-polygon), and picks the smallest one -- correct regardless of
 * winding convention, and "smallest enclosing loop" is also just the right
 * answer if rooms are ever nested.
 */
interface Vec2 {
  readonly x: number;
  readonly z: number;
...
/** The smallest closed wall loop containing `click`, or `undefined` if no enclosed area was found there. */
function findEnclosingRoom(ctx: ToolContext, click: ConstructionPosition): DerivedRoom | undefined {
