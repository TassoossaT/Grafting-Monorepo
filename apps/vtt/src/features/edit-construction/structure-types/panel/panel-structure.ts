...
import type { CascadeContext, EditRole, MotionContext, ReshapeContext, RolePolicy, StructureTrait, StructureTypeDefinition, StructureView } from "../structure-type.ts";
...
import { panelRailOf } from "../../topology/panel-rail.ts";
...
/**
 * How an opening's own rim rides a host wall that just *deformed* instead of
 * translated -- a single corner drag, say, which stretches one side of the
 * panel while the other stays put. There is no single delta that describes
 * that for the opening (the wall is no longer the same shape it was), so
 * this reprojects each of the opening's own corners through the same
 * travel-and-height rail `rimCorners`/`panelRailOf` place one from in the
 * first place: read where the corner used to sit along the wall's *old*
 * rail, then put it at that same travel on the wall's *new* one, keeping
 * its own height (a rigid translation already carries height correctly
 * through `panelMotionInfluences`'s own `transport` link, unconditionally
 * on the Y axis -- this only ever fires for a corner that link left alone).
 *
 * Only reached for a corner the solver did not already resolve:
 * `edit-orchestrator.ts` never lets a `deriveMotion` result override an
 * already-resolved node, so a rigid translation's own carry is untouched by
 * this -- it only fills in the gap a deformation leaves.
 */
function deriveOpeningMotion(
  ...
  const { graphSnapshot, contourPort } = context;
  if (!graphSnapshot || !contourPort) return derived;
  const standing = new Map(graphSnapshot.nodes.map((node) => [node.id, node.position]));
  const at = (id: string): ConstructionPosition | undefined => positions.get(id) ?? standing.get(id);

  const openings = topologies.filter((topology) => topology.surfaceType === openingStructureType.surfaceType);
  if (openings.length === 0) return derived;
  const walls = topologies.filter((topology) => hasTrait(topology.surfaceType, "partition"));

  for (const opening of openings) {
    const firstEdgeId = opening.outerLoops[0]?.[0]?.edgeId;
    if (firstEdgeId === undefined) continue;
    const wall = walls.find((candidate) => candidate.holes.some((loop) => loop.some((use) => use.edgeId === firstEdgeId)));
    if (wall === undefined) continue;

    const oldRail = panelRailOf(contourPort, wall);
    if (oldRail === undefined) continue;
    const movedWall: ConstructionRegionTopology = {
      ...wall,
      nodes: wall.nodes.map((node) => ({ id: node.id, position: at(node.id) ?? node.position })),
    };
    const newRail = panelRailOf(contourPort, movedWall);
    if (newRail === undefined) continue;

    for (const node of opening.nodes) {
      if (positions.has(node.id)) continue;
      const travel = oldRail.travelTo(node.position);
      derived.set(node.id, newRail.positionAt(travel, node.position.y));
    }
  }
  return derived;
}
...
export const openingStructureType: StructureTypeDefinition = Object.freeze({
  ...
  interactionOver: panelInteractionOver,
  deriveMotion: deriveOpeningMotion,
});
