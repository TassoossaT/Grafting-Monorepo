      ...
      activeTool === "roof" ? (
        ...
        <div style={{ display: "grid", gap: "0.6rem" }}>
          ...
          {params["platform-contour"].shape === "spiral" && <>
            ...
            <label>Subida <input type="number" step="0.1" value={params["platform-contour"].rise ?? 3} onChange={(event) => onParamsChange("platform-contour",{ ...params["platform-contour"],rise:Number(event.currentTarget.value) })} /></label>
          </>}
          {(params["platform-contour"].shape === "slope" || params["platform-contour"].shape === "spiral") && <p>Rampa: clique nos pontos do eixo, cada um na altura de onde clicar, e clique de novo no último para concluir. Espiral: clique no centro. Uma ponta que cai na borda de uma plataforma na mesma altura é soldada a ela.</p>}
