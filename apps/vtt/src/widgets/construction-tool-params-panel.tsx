    ...
    content:
      activeTool === "roof" ? (
        ...
        <div style={{ display: "grid", gap: "0.6rem" }}>
          ...
          <div className="gm-material-grid">
            {(["rectangle", "circle", "polygon", "freehand", "slope", "spiral"] as const).map((shape,i) => <SelectableChip key={shape} label={["Retângulo", "Círculo", "Polígono", "Livre / curvas", "Rampa / curva", "Espiral"][i]!} swatchColor="#79b8e8" selected={(params["platform-contour"].shape ?? "rectangle") === shape} onSelect={() => onParamsChange("platform-contour",{ ...params["platform-contour"],shape })} />)}
          </div>
          {(params["platform-contour"].shape === "slope" || params["platform-contour"].shape === "spiral") && <label>Largura <input type="number" min="0.1" step="0.1" value={params["platform-contour"].width ?? 1.5} onChange={(event) => onParamsChange("platform-contour",{ ...params["platform-contour"],width:Number(event.currentTarget.value) })} /></label>}
          {params["platform-contour"].shape === "spiral" && <>
            <label>Raio <input type="number" min="0.5" step="0.1" value={params["platform-contour"].radius ?? 2.5} onChange={(event) => onParamsChange("platform-contour",{ ...params["platform-contour"],radius:Number(event.currentTarget.value) })} /></label>
            <label>Voltas <input type="number" min="0.25" step="0.25" value={params["platform-contour"].turns ?? 1} onChange={(event) => onParamsChange("platform-contour",{ ...params["platform-contour"],turns:Number(event.currentTarget.value) })} /></label>
          </>}
          {(params["platform-contour"].shape === "slope" || params["platform-contour"].shape === "spiral") && <label>Subida <input type="number" step="0.1" value={params["platform-contour"].rise ?? 3} onChange={(event) => onParamsChange("platform-contour",{ ...params["platform-contour"],rise:Number(event.currentTarget.value) })} /></label>}
          {(params["platform-contour"].shape === "slope" || params["platform-contour"].shape === "spiral") && <p>Rampa: arraste do início ao fim; ela começa na altura de onde você clicou e sobe o valor de Subida. Espiral: clique no centro. Uma ponta que cai na borda de uma plataforma na mesma altura é soldada a ela.</p>}
