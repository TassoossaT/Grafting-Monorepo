...
| E3.3 | Unified brush/mutation engine: node operations (move/add/delete/merge/split, per the revised `ADR-0022`) for **every** construction domain — terrain and walls/doors/windows alike, as sets of `grafting-graph-core` graph nodes. **Revised 2026-08-12**: `apply_cell_patch` as a separate terrain-only algorithm does not exist — `ADR-0022`'s "Terrain/structure seam: resolved" section closes that split; terrain cells are `Surface`s like any other, and `PrismCellAssignment`/"rotation" are generation-time inputs, not a parallel structure or a graph primitive. The "6-slot neighborhood" language elsewhere in this doc and in `vtt-map-construction-roadmap.md` is retired — it described `PrismGridMesh`'s old fixed-grid adjacency, not a rule of the graph operation | **Node operations done.** Mutation primitives, `Surface`/`SurfaceRegistry`, and all five `ADR-0022` operations (`move_node`/`delete_node`/`merge_surfaces`/`split_surface`/`duplicate_surface`, `libs/graph/core/src/construction.rs`) have landed, all sharing the same validate-before-any-mutation discipline. `duplicate_surface` generates its connecting edges automatically as a closed ring over the caller-supplied new nodes (confirmed with the owner), rather than requiring an arbitrary caller-supplied edge list. Still open, and the last real piece of E3.3: terrain and wall **generation** code — deriving node cycles from a chosen module's face (terrain) or from a door/wall's own placement logic (structure), each isolated per `ADR-0022`'s generation-isolation requirement, feeding these now-complete node operations rather than reinventing them | Alta | Alto — this is "the bulk of construction" the owner wants finished |
