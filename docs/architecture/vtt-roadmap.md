...
| E1.3 | Consolidate `/lab`: migrate standalone trials into node kinds on the bench registry (several already have equivalents — `heightmap.perlin`, `terrain.discretize`, `grid.irregular`, `terrain.stack`); delete `/lab/trials` once confirmed. `vtt-brush` is flagged as a likely exception (interactive editor, not a data-transform node) — full send-ready prompt below, including that `vtt-brush`'s wall model is now stale a second time | **Done** — pronta. Trabalho de organização do Architecture Studio, sem depender do mapa do VTT. | Média | Médio — tooling hygiene, not blocking; `vtt-brush`'s own redesign is what's actually higher-stakes |
| E1.4 | Fix `/lab/vtt-brush`'s wall mesh: currently extrudes a solid box (`thickness` offset + 5 faces) instead of a flat double-sided plane; material is already `doubleSided: true`, so the fix is dropping the box extrusion, not adding anything | **Done** — pronta. Pequena e isolada, embora visualmente relacionada ao mapa experimental. | Baixa | Baixo — cosmetic, isolated to one lab trial |
...
| E3.7 | Edit-mode interaction layer in `apps/vtt`: pointer capture, tool/brush selection state, drag-to-move a node, trigger generate-terrain-cell/generate-wall actions, undo/redo. `apps/architecture-studio/src/app/lab/vtt-brush` (1132 lines) is a real interactive reference for the UX, but its data model (`BoundarySegment`/`TerrainSurfaceKind`/`CellId`) predates `ADR-0022`'s node-graph revision — reuse the interaction patterns, rewrite the data model against `Surface`/`NodeId`, do not port directly | **Done — task `VTT-MAP-EDIT-MODE`.** Closed the remaining node-position/picking gaps first (`ConstructionSessionPort.getNodePositions()`, `entities/map` node tracking, `SceneRenderPort.pick`), then built pointer-capture drag-to-move, a move-node undo/redo stack, and generate-terrain-cell/generate-wall UI triggers. Browser-verified the full loop live (drag reshapes the mesh in real time, undo/redo, both generate buttons) — see `apps/vtt/notes/0005-edit-mode-interaction.md`, which also records a real `@grafting/render-3d` picking bug found and fixed (`View.pick`'s probe camera never got `updateMatrixWorld`, so every ray was cast from the world origin) and one carried-forward gap (dragging off the edge of existing map geometry doesn't resolve a target, no ground-plane fallback yet) | Alta | Alto — this is "edit mode" as the owner asked for it |
| E3.8 | End-to-end validation slice: inside `apps/vtt`, generate a terrain cell and a wall with a door, see both rendered, drag a node in edit mode, see the change propagate live. This is the epic's own acceptance criterion, not a separate feature | **Done — task `VTT-E2E-VALIDATION`.** Running the checklist live surfaced a real bug the prior two tasks' own "browser-verified" claims had missed: every wall/door surface ever generated silently failed to triangulate and was absent from `entities/map`, because `grafting-procgen-structure-generation`'s wall height was added to the wrong axis (collapsed onto the centerline for any wall running along Z, which is exactly how this app's own seed and edit-mode trigger build walls) — invisible until now because unlit surfaces already render solid black (`E3.9`), so the terrain cell's own black quad was mistaken for a rendered wall. Fixed (`wall.rs`'s `position_at`), regression-tested, then the full checklist re-verified live in a real browser: terrain + wall-with-door both render as 4 real triangulated surfaces, drag-to-move reshapes the mesh live, undo/redo both work. See `apps/vtt/notes/0006-e2e-validation-slice.md`, which also flags that this crate's own Rust test suite (`#[wasm_bindgen_test]`) does not run in CI or under `cargo test` at all — a separate, still-open gap | Média | Alto — this is the actual "posso ver e mexer no mapa" answer |
| E3.9 | Render-correctness follow-up: configure lighting for map surfaces and resolve the measured terrain PrismGridMesh/render-space axis mismatch; repeat browser verification with terrain and wall fixtures before calling Epic 3 complete | **Done — task `VTT-RENDER-CORRECTNESS`.** Lighting: `Render3dSceneAdapter.start()` passed no `lights` to `createEngine`, so every `"lit"`-material map surface rendered solid black regardless of color — added one ambient + one directional light. Axis: `grafting-procgen-terrain-generation`'s `generate_terrain_cell_surface` handed `PrismGridMesh`'s own mesh-space positions (that crate's documented convention: Z is vertical) straight through, uncorrected, into a construction-graph the app renders Y-up — a flat terrain cell wasn't flat in the app's own ground plane. Remapped once at that boundary (mesh-space `[x,y,z]` → render-space `[x,z,y]`), not inside `PrismGridMesh` itself, so the grid's own general convention stays intact for its other consumer (`generation-wasm`). Browser-verified live via ground-truth node positions: terrain corners now flat at one render-space Y with a real X/Z footprint, coordinate-consistent with the seeded wall for the first time; drag/undo regression-checked, no app console errors. See `apps/vtt/notes/0007-render-correctness-follow-up.md` | Média | Alto — a correct edit pipeline is not useful if map geometry renders black or on the wrong axes |
