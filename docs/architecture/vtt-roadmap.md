...
| E3.3 | Unified brush/mutation engine: node operations (move/add/delete/merge/split, per the revised `ADR-0022`) for **every** construction domain — terrain and walls/doors/windows alike, as sets of `grafting-graph-core` graph nodes. **Revised 2026-08-12**: `apply_cell_patch` as a separate terrain-only algorithm does not exist — `ADR-0022`'s "Terrain/structure seam: resolved" section closes that split; terrain cells are `Surface`s like any other, and `PrismCellAssignment`/"rotation" are generation-time inputs, not a parallel structure or a graph primitive. The "6-slot neighborhood" language elsewhere in this doc and in `vtt-map-construction-roadmap.md` is retired — it described `PrismGridMesh`'s old fixed-grid adjacency, not a rule of the graph operation | In progress — mutation primitives, `Surface`/`SurfaceRegistry`, and `move_node`/`delete_node`/`merge_surfaces`/`split_surface` (`libs/graph/core/src/construction.rs`) have all landed. `delete_node` implements `ADR-0022`'s general cycle-repair algorithm in full; `merge_surfaces`/`split_surface` take caller-supplied `SurfaceSpec`s (only the domain knows how to join/cut geometry) and share the same before-any-mutation validation discipline. Still open: `Duplicate` (needs the caller to also supply new edges — `Surface` doesn't track which edges form its boundary, a different shape of problem from `Merge`/`Split`); terrain and wall generation code, each isolated per `ADR-0022`'s generation-isolation requirement | Alta | Alto — this is "the bulk of construction" the owner wants finished |
