...
| DEC-048 | Provider-neutral coordination uses `tools/ia-graft`, isolated worktrees and Git/PR evidence; chat and duplicate ownership/handoff files are not authoritative (ADR-0010). |
...
| DEC-053 | Agents may create forward-only commits only on their own `task/<TASK-ID>` worktree and push that branch; they never write the default branch, rewrite history, force refs or merge PRs (ADR-0015). |
