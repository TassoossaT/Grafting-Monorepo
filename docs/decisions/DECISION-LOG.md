...
| DEC-053 | Agents may create forward-only commits only on their own `task/<TASK-ID>` worktree and push that branch; a clean task may temporarily occupy a clean main checkout only through `ia-graft task checkout` for testing, never commits; agents never write the default branch, rewrite history, force refs or merge PRs (ADR-0015). |
